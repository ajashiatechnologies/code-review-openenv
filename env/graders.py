...
EPS = 0.10      # Minimum = 0.1 as per Reddit tip


def normalize_score(score: float) -> float:
    """Follows Reddit advice: min 0.1, max 0.99"""
    score = float(score)
    
    if score <= 0.0:
        return EPS                  # 0.10
    if score >= 0.99:
        return 0.99 - 0.01          # 0.98
    
    score = max(EPS, min(0.99 - 0.01, score))
    
    if score <= 0.0 or score >= 0.99:
        return 0.5
